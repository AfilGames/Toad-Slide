	...
	{		
		...
		if (!_event_ctrl.animation_completed)
		{
			...
			if (!_event_ctrl.value_calculated)
			{
				...
				//show_debug_message($"{_node.animator.motion_state} : {_node.name} Value from: {_event_ctrl.value_start} Value to: {_event_ctrl.value_to} Value diff {_event_ctrl.value_diff}")
