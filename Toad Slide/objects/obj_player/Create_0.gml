	...
	grab = function() {
		if !instance_exists(grab_object) {
			...
			while(_length <= grab_range) {
				...
				var _found_objective = grid_data.check_objective(_x, _y);
				var _container = instance_exists(_found_objective) and (_found_objective.objective_type == "container");
				
				if (_container and (_length > 0)) {
					_objective = _found_objective;
				}
				...
				if (instance_exists(_pushable)) {
					...
					_element.anim_offset.x = _element.x - _target_x;
					_element.anim_offset.y = _element.y - _target_y;
...
draw_element = function() {
	...
	draw_sprite_ext(
		...
		(-image_xscale * x_side) * (anim_xscale * anim_master_scale),
