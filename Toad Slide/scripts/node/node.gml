...
function node(_name, _origin = NODE_ORIGIN.MIDDLE_CENTER) constructor
{
	...
	static node_add = function(_nodes)
	{
		...
		{
			var _element = argument[_i];
			_element.id = array_length(nested_nodes);
			array_push(nested_nodes, variable_clone(_element));	
			array_push(nested_depth_order, variable_clone(_element.id));
			
			if (_element.transform.size.x > transform.fixed.size.x && _element.transform.size.y > transform.fixed.size.y)
			{
				node_set_size(_element.transform.fixed.size.x, _element.transform.fixed.size.y);	
			}
	...
	static node_set_position = function(_x, _y)
	{
		...
		transform.local.position.x = round(_x);
		transform.local.position.y = round(_y);	
	...
	static node_set_scale = function(_x, _y)
	{
		...
		transform.local.scale.x = _x;
		transform.local.scale.y = _y;
	...
	static node_transform_rotate = function(_rotation)
	{
		...
		transform.local.rotation += _rotation;
		__rotate_nested();
	}
	...
	{
		transform.local.position.x += _x;
		transform.local.position.y += _y;
 ...
 function __node_transform() : MiniTransform() constructor
{
	...
	static __containter_update = function(_x, _y)
	{			
